...
def weight_prediction():
    ...
    # Model training
    try:
        # Prepare input features (all five parameters)
        ...
        # Random Forest models for each target
    ...
    with st.expander("📝 Prediction Guide", expanded=True):
        ...
        3. View predicted results for Crude Weight, Purified Weight, and Yield
    ...
    with st.form("prediction_form"):
        ...
        if st.form_submit_button("🔮 Run Prediction", type="primary"):
            try:
                ...
                if not valid:
                    st.error("Cannot make prediction - some parameters are outside the trained data range")
                    return
                
                # Prepare input with all five features
                X_input = pd.DataFrame([[
                    input_data['p-aminophenol(g)'],
                    input_data['Acetic Anhydride(ml)'],
                    input_data['PA:AA'],
                    input_data['Reaction time(min)'],
                    input_data['T(°C)']
                ]], columns=['p-aminophenol(g)', 'Acetic Anhydride(ml)', 'PA:AA', 'Reaction time(min)', 'T(°C)'])
                ...
                with res_cols[0]:
                    st.metric("Crude Product Weight", f"{crude_pred:.2f} g")
                with res_cols[1]:
                    st.metric("Purified Product Weight", f"{purify_pred:.2f} g")
                with res_cols[2]:
                    st.metric("Yield", f"{yield_pred:.1f}%")
                ...
                with st.expander("📊 Details", expanded=False):
                    ...
                    **Model Info**:
                    ...
                    - Using all five input parameters for prediction
